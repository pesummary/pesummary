...
class TGRWebpageGeneration(_WebpageGeneration):
    ...
    **kwargs: dict
    ...
    def make_imrct_pages(self, make_diagnostic_plots=False):
        ...
        if make_diagnostic_plots:
            desc = (
                "Below we show additional plots generated for the IMR consistency " "test"
            )
            html_file.make_banner(
...
def main(args=None):
    ...
    if opts.test == "imrct":
        ...
        if opts.cutoff_frequency is not None:
            ...
            logger.info(
                "No Cutoff Frequency supplied on command line. Checking in the samples files."
            )
            try:
                if float(open_files["inspiral"].config["maximum-frequency"]) == float(
                    open_files["postinspiral"].config["minimum-frequency"]
                ):
                    data["Cutoff Frequency"] = float(
                        open_files["inspiral"].config["maximum-frequency"]
                    )
                else:
                    logger.warning(
                        "The minimum and maximum frequencies for inspiral and postinspiral do not match!"
                    )
                    data["Cutoff Frequency"] = None
            except (AttributeError, KeyError):
                logger.info(
                    "No Cutoff Frequency information in supplied samples file. Setting to None."
                )
                data["Cutoff Frequency"] = None
