...
def generate_imrct_deviation_parameters(samples_dict, **kwargs):
    ...
    imrct_deviations = imrct_deviation_parameters_from_final_mass_final_spin(
        ...
        samples_dict["inspiral"]["final_spin_non_evolved"],
...
def make_imrct_plots(imrct_deviations, samples_dict, webdir=None):
    ...
    for key in ["inspiral", "postinspiral"]:
        ...
        fig.savefig(os.path.join(plotdir, base_string.format("final_mass_final_spin", key)))
        fig, _, _, _ = samples_dict[key].plot(
            ["mass_1", "mass_2"], type="triangle", smooth=4, fill_alpha=0.2, labels=[key]
        )
        fig.savefig(os.path.join(plotdir, base_string.format("mass_1_mass_2", key)))
        fig, _, _, _ = samples_dict[key].plot(["a_1", "a_2"], type="triangle", smooth=4, fill_alpha=0.2, labels=[key])
        fig.savefig(os.path.join(plotdir, base_string.format("a_1_a_2", key)))
