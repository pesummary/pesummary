...
# get the DINGO model
DINGO_PT_FILE=$(curl -sSL https://webdav.tuebingen.mpg.de/dingo-ci/ | grep -oE 'href="[^"]+\.pt"' | sed 's/href="//;s/"//' | head -n 1)
curl -sSLO "https://webdav.tuebingen.mpg.de/dingo-ci/$DINGO_PT_FILE"

# download an ini file to run basic inference on 
curl -sSLO https://git.ligo.org/nihar.gupte/dingo-ci/-/blob/master/GW150914.ini
